...
import { collection, addDoc, getDocs, doc, deleteDoc } from "firebase/firestore"; // Importe o deleteDoc aqui
...
const App: React.FC = () => {
 ...
 // 1. CARREGAR USUÁRIOS (Corrigido)
  useEffect(() => {
    const carregarUsuarios = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "usuarios"));
        const usuariosFirebase = querySnapshot.docs.map(doc => ({
          ...doc.data(),
          id: doc.id // Importante: pegar o ID real do Firestore
        })) as User[];

        if (usuariosFirebase.length > 0) {
 ...
 // 2. ADICIONAR USUÁRIO (Refatorado)
  const updateAllUsers = async (newUsers: User[]) => {
    try {
      // Pega o usuário que acabou de ser criado (o último do array vindo do Dashboard)
      const newUserRaw = newUsers[newUsers.length - 1];
      
      // Salva no Firebase
      const docRef = await addDoc(collection(db, "usuarios"), newUserRaw);
      
      // Atualiza o estado local com o ID gerado pelo Firebase
      const newUserWithId = { ...newUserRaw, id: docRef.id };
      setAllUsers(prev => [...prev.filter(u => u.id !== undefined), newUserWithId]);
...
setError('E-mail ou senha incorretos. Verifique suas credenciais.');
 ...
 // 3. DELETAR USUÁRIO (Movido para o App para passar via props se necessário)
